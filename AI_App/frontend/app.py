...
def call_log(user_prompt, generated_code, executed_code, result_summary, error=""):
    try:
...
with st.sidebar:
    ...
    if st.button("🗑️ Xóa lịch sử chat", use_container_width=True):
        for k in ["messages", "pending_code", "explanation", "original_code", "last_result"]:
...
if st.session_state.pending_code is not None:
    ...
    edited_code = st.text_area(
        "Review và chỉnh sửa code Python:",
    ...
    if approve:
        with st.spinner("⚙️ Đang thực thi..."):
            result = call_execute(edited_code)
...
if st.session_state.last_prompt and st.session_state.pending_code is None:
    ...
    if result and result.get("code"):
        ...
        st.session_state.messages.append({
            "role": "assistant",
            "content": f"⚠️ Không thể sinh code: {result.get('explanation', 'Unknown error')}"
        })
    st.rerun()
