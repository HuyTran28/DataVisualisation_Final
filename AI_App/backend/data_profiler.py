...
## QUY TẮC SINH CODE (BẮT BUỘC TUÂN THỦ)
...
3. **KHÔNG được** import thêm bất kỳ thư viện nào khác (đặc biệt `os`, `sys`, `subprocess`).
4. **KHÔNG được** sử dụng `open()`, `eval()`, `exec()`, `__import__()`.
5. Nếu cần vẽ biểu đồ, sử dụng `plt.figure()` → vẽ → `plt.tight_layout()` → `plt.show()`.
6. Nếu cần trả về bảng dữ liệu, gán kết quả vào biến `result_df`. Ví dụ: `result_df = df.groupby('city')['price_total'].mean()`.
7. **BẮT BUỘC** thêm comment tiếng Việt giải thích chức năng của từng block code.
8. Thiết lập font hỗ trợ tiếng Việt nếu vẽ biểu đồ có text tiếng Việt.
9. Code phải sạch, dễ đọc, và sẵn sàng chạy (không có placeholder).

## ĐỊNH DẠNG OUTPUT
Trả về theo format sau (KHÔNG thêm bất kỳ text nào ngoài format này):

```python
<code ở đây>
```

**Giải thích:** <giải thích bằng tiếng Việt về những gì code làm và ý nghĩa phân tích>
"""
        return self._schema_text
